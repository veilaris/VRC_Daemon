...
> Локальная нейросеть для обнаружения игрока на скриншоте. Работает быстрее LLM Vision, но требует NVIDIA GPU (от 3 ГБ VRAM). Первый запуск автоматически скачает веса (~700 МБ).
> ```bash
> pip install torch torchvision --index-url https://download.pytorch.org/whl/cu128
> pip install groundingdino-py
> ```
> Без DINO трекер движения работает через LLM Vision. Команда «дверь» работает всегда — она ищет синий контур взаимодействия VRChat напрямую.
...
| зайди в портал / иди в портал | Войти в портал — важно: ставь портал так, чтобы твой персонаж был между порталом и ботом |
